...
class HandlerTests(unittest.TestCase):
    ...
    def test_homepage_creates_schema_and_returns_html(self):
        ...
        try:
            ...
            self.assertIn(landing.HEADLINE_SHORT.encode(), body)
            self.assertIn(landing.CTA_SHORT.encode(), body)
    ...
    def test_homepage_uses_long_copy_when_layout_has_room(self):
        ...
        try:
            ...
            self.assertIn(landing.HEADLINE_LONG.encode(), body)
            self.assertIn(landing.CTA_LONG.encode(), body)
            self.assertIn(b"hero-short", body)
            self.assertIn(b"cta-short", body)
        finally:
            httpd.shutdown()
            httpd.server_close()
